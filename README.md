...
- **BaseURL, API 키 보안**: BaseURL과 API 키를 안전하게 관라하기 위해 `.gitignore` 로 커밋 예외 처리
...
- **Query String 파라미터 설정으로 `Overfetching` 방지**: Query String 파라미터 중 `exclude=minutely,alerts` 설정으로 불필요한 `minutely`와 `alerts` 데이터 호출 제외 후 API 호출
